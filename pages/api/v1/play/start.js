...
import { resolveVersioned, withApiVersion } from '../../../../lib/middleware/apiVersion';
...
export default withApiVersion(async function handler(req, res) {
  ...
  try {
    ...
    const started = await withOrganization(organizationId, async () => {
      const engine = await getEngineByMode(mode);
      return engine.startSession(organizationId, deckTag);
    });

    await registerPlaySession(started.playId, organizationId, mode);
    ...
    const code =
      err.appCode || (status === 400 ? ERROR_CODES.VALIDATION_ERROR : ERROR_CODES.SYSTEM_FAILURE);
