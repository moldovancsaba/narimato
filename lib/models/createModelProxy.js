...
function getTenantModels() {
  return require('../tenantContext').getTenantModels();
}

/**
 * Mongoose model proxy: uses tenant models when inside withOrganization(), else legacy default connection.
 */
function createModelProxy(name, registerOnConnection, getLegacyModel, staticProps = {}) {
  ...
  return new Proxy(proxyTarget, {
    get(_target, prop) {
      ...
      if (Object.prototype.hasOwnProperty.call(staticProps, prop)) {
