...
const playSchema = new mongoose.Schema({
  ...
  state: { type: String, enum: ['swiping', 'voting', 'vote_only', 'child_session'], default: 'swiping' },
  mode: { type: String, enum: ['classic', 'swipe_only', 'swipe_more', 'vote_only'], default: 'classic' },
  ...
  // Vote-only mode fields (temporary arena and vote audit)
  unrankedDeck: [String],
  rankedDeck: [String],
  // FUNCTIONAL: Track active challenger/opponent for strict vote-only flow
  // STRATEGIC: Ensures each challenger continues comparisons until placement is resolved
  activeChallenger: { type: String },
  activeOpponent: { type: String },
  voteHistory: [{
    winner: String,
    loser: String,
    timestamp: { type: Date, default: Date.now }
...
const { createModelProxy, legacyModel } = require('./createModelProxy');

function registerPlayModel(connection) {
  return connection.models.Play || connection.model('Play', playSchema);
}

module.exports = createModelProxy('Play', registerPlayModel, () => legacyModel('Play', playSchema));
module.exports.playSchema = playSchema;
module.exports.registerPlayModel = registerPlayModel;
