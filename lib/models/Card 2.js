...
const cardSchema = new mongoose.Schema({
  ...
  // Ordering
  // FUNCTIONAL: Presentation order within a parent deck (lower numbers appear earlier)
  // STRATEGIC: Allows deterministic onboarding sequences without new models or endpoints
  ...
  // Onboarding control
  // FUNCTIONAL: Flags parent/root cards whose children should run as an onboarding segment
  // before any selected deck across the organization (right-only Swipe onboarding)
  // STRATEGIC: Reuses existing onboarding engine without new models/endpoints; default false
  // maintains legacy behavior and avoids data backfill needs
...
// Index to efficiently find onboarding parents within an org and their decks
cardSchema.index({ organizationId: 1, parentTag: 1, isOnboarding: 1 });
// Index to support ordered child retrieval within a deck
cardSchema.index({ organizationId: 1, parentTag: 1, sortIndex: 1 });
