...
const swipeOnlyPlaySchema = new mongoose.Schema({
  ...
  // Onboarding mode flag (right-only, no ranking persisted)
  ...
  // Onboarding progress pointer (no swipes recorded for onboarding)
  onboardingIndex: { type: Number, default: 0 },
...
function registerSwipeOnlyPlayModel(connection) {
  return connection.models.SwipeOnlyPlay || connection.model('SwipeOnlyPlay', swipeOnlyPlaySchema);
}

module.exports = createModelProxy('SwipeOnlyPlay', registerSwipeOnlyPlayModel, () =>
  legacyModel('SwipeOnlyPlay', swipeOnlyPlaySchema)
);
module.exports.registerSwipeOnlyPlayModel = registerSwipeOnlyPlayModel;
