...
class RankOnlyEngine {
  ...
  async handleInput(playId, { action, payload }) {
    const ro = await RankOnlyPlay.findOne({ [fieldNames.PlayUUID]: playId, status: 'active' });
    if (!ro) {
    ...
    if (ro.phase === 'swipe') {
      ...
      if (res.completed) {
        ...
        const swipeDoc = await SwipeOnlyPlay.findOne({ [fieldNames.PlayUUID]: ro.swipeSessionId });
        const likedIds = (swipeDoc?.likedCards || []).map(l => l[CARD_FIELDS.ID]);
  ...
  async getNext(playId) {
    const ro = await RankOnlyPlay.findOne({ [fieldNames.PlayUUID]: playId });
    if (!ro) {
  ...
  async getResults(playId) {
    ...
    if (ro.phase === 'vote' && ro.voteSessionId) {
      ...
      return {
        ...
        personalRanking: voteRes[VOTE_FIELDS.PERSONAL_RANKING],
    ...
    const cards = await Card.find({ [fieldNames.CardUUID]: { $in: liked } });
    const map = new Map(cards.map(c => [c[fieldNames.CardUUID], { id: c[fieldNames.CardUUID], title: c.title, description: c.description, imageUrl: c.imageUrl }]));
