...
class SwipeOnlyEngine {
  ...
  async createSession(organizationId, deckTag, options = {}) {
    try {
      ...
      const cardIds = presentationCards.map(card => card[CARD_FIELDS.UUID]);
      ...
      console.log(`✅ SwipeOnly session created: ${session[CARD_FIELDS.UUID]} with ${cards.length} cards`);
  ...
  async processSwipe(playId, cardId, direction) {
    try {
      ...
      if (session.swipes.some(swipe => swipe[CARD_FIELDS.ID] === cardId)) {
      ...
      const swipedCardIds = session.swipes.map(s => s[CARD_FIELDS.ID]);
  ...
  async getCurrentCard(playId) {
    try {
      ...
      const cardDetails = await Card.findOne({ [CARD_FIELDS.UUID]: currentCardId });
      ...
      return {
        playId: session[CARD_FIELDS.UUID],
        currentCard: {
          id: cardDetails[CARD_FIELDS.UUID],
  ...
  async getFinalRanking(playId) {
    try {
      ...
      for (const likedCard of session.likedCards) {
        const cardDetails = await Card.findOne({ [CARD_FIELDS.UUID]: likedCard[CARD_FIELDS.ID] });
        if (cardDetails) {
          rankedCards.push({
            rank: likedCard.rank,
            card: {
