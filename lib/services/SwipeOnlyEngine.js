...
class SwipeOnlyEngine {
  ...
  static async startSession(organizationId, deckTag) {
    ...
    return await engine.createSession(organizationId, deckTag, { isOnboarding: false });
  }
   ...
   */
  async createSession(organizationId, deckTag, options = { isOnboarding: false }) {
    try {
      ...
      if (!options.isOnboarding) {
        // FUNCTIONAL: Shuffle cards for random presentation order (standard swipe-only)
        // STRATEGIC: Prevent order bias in swipe decisions
        presentationCards = [...cards].sort(() => Math.random() - 0.5);
      } // else: deterministic order by globalScore from the initial query
  ...
  async processSwipe(playId, cardId, direction) {
    try {
      ...
      // Onboarding: do not record swipes or rankings; just advance by index
      if (session.isOnboarding) {
        const nextIndex = (session.onboardingIndex || 0) + 1;
        session.onboardingIndex = nextIndex;
        let result = {
          success: true,
          completed: false,
          nextCardId: session.cardIds[nextIndex] || null,
        };
        if (nextIndex >= session.cardIds.length) {
          result.completed = true;
          result.nextCardId = null;
      ...
      // FUNCTIONAL: Record the swipe action (standard mode)
      session.swipes.push({
        cardId,
        direction,
        timestamp: new Date()
  ...
  async getCurrentCard(playId) {
    try {
      ...
      // Onboarding uses index-based progression (no swipes)
      if (session.isOnboarding) {
        const idx = session.onboardingIndex || 0;
        if (idx >= session.cardIds.length) {
          return { completed: true };
        }
        const currentCardId = session.cardIds[idx];
        const cardDetails = await Card.findOne({ uuid: currentCardId });
        if (!cardDetails) throw new Error('Current card not found');
        return {
          playId: session.uuid,
          currentCard: {
            id: cardDetails.uuid,
      ...
      // Standard mode uses remaining-swipes calculation
      const swipedCardIds = session.swipes.map(s => s.cardId);
      const remainingCards = session.cardIds.filter(id => !swipedCardIds.includes(id));
  ...
  /**
   * Advance onboarding without recording swipes
   */
  async advanceOnboarding(playId) {
    const session = await SwipeOnlyPlay.findOne({ uuid: playId, status: 'active' });
    if (!session || !session.isOnboarding) {
      const err = new Error('Onboarding session not found or not active');
      err.statusCode = 400;
      throw err;
    }
    const nextIndex = (session.onboardingIndex || 0) + 1;
    session.onboardingIndex = nextIndex;
    let result = { success: true, completed: false, nextCardId: session.cardIds[nextIndex] || null };
    if (nextIndex >= session.cardIds.length) {
      result.completed = true;
      result.nextCardId = null;
      session.status = 'completed';
