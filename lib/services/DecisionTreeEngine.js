...
class DecisionTreeEngine {
  ...
  async createSession(organizationId, deckTag) {
    try {
      ...
      const session = new Play({
        [CARD_FIELDS.UUID]: uuidv4(),
        organizationId,
        deckTag,
        cardIds: shuffled.map(c => c[CARD_FIELDS.UUID]),
        swipes: [],
        votes: [],
        [VOTE_FIELDS.PERSONAL_RANKING]: [],
        status: 'active',
        state: 'swiping'
      });
      ...
      console.log(`✅ New session: ${session[CARD_FIELDS.UUID]} with ${cards.length} cards`);
      
        return {
          playId: session[CARD_FIELDS.UUID],
          deckTag,
          totalCards: cards.length,
          cards: shuffled.map(c => ({
            id: c[CARD_FIELDS.UUID],
            title: c.title,
            description: c.description,
            imageUrl: c.imageUrl
          })),
          currentCardId: shuffled[0][CARD_FIELDS.UUID]
        };
    } catch (error) {
  ...
  async processSwipe(playId, cardId, direction) {
    try {
      const session = await Play.findOne({ [CARD_FIELDS.UUID]: playId, status: 'active' });
      if (!session) {
      ...
      if (direction === 'right') {
        if (session[VOTE_FIELDS.PERSONAL_RANKING].length === 0) {
          // First card - direct add
          session[VOTE_FIELDS.PERSONAL_RANKING].push(cardId);
          ...
          result.votingContext = {
            newCard: cardId,
            compareWith: this.getComparisonCard(session[VOTE_FIELDS.PERSONAL_RANKING])
          };
      ...
      // Find next card
      const swipedIds = session.swipes.map(s => s[CARD_FIELDS.ID]);
  ...
  async processVote(playId, cardA, cardB, winner) {
    try {
      ...
      if (!session) {
        ...
        const anySession = await Play.findOne({ [CARD_FIELDS.UUID]: playId });
        if (anySession) {
      ...
      const newCard = session[VOTE_FIELDS.PERSONAL_RANKING].includes(cardA) ? cardB : cardA;
      const existingCard = session[VOTE_FIELDS.PERSONAL_RANKING].includes(cardA) ? cardA : cardB;
      
      // Position the new card based on vote result
      if (winner === newCard) {
        // New card wins - insert before existing card
        const index = session[VOTE_FIELDS.PERSONAL_RANKING].indexOf(existingCard);
        session[VOTE_FIELDS.PERSONAL_RANKING].splice(index, 0, newCard);
      } else {
        // New card loses - insert after existing card
        const index = session[VOTE_FIELDS.PERSONAL_RANKING].indexOf(existingCard);
        session[VOTE_FIELDS.PERSONAL_RANKING].splice(index + 1, 0, newCard);
      }
      
      console.log(`📊 Updated ranking: [${session[VOTE_FIELDS.PERSONAL_RANKING].join(', ')}]`);
  ...
  async getSessionData(playId) {
    try {
      ...
      for (const cardId of session.cardIds) {
        const card = await Card.findOne({ [CARD_FIELDS.UUID]: cardId });
        if (card) {
          cards.push({
      ...
      if (session.state === 'voting') {
        ...
        if (lastSwipe && lastSwipe.direction === 'right') {
          votingContext = {
            newCard: lastSwipe[CARD_FIELDS.ID],
            compareWith: this.getComparisonCard(session[VOTE_FIELDS.PERSONAL_RANKING])
          };
        }
      } else {
  ...
  async processHierarchical(session) {
    try {
      ...
      for (const cardId of session[VOTE_FIELDS.PERSONAL_RANKING]) {
        const card = await Card.findOne({ [CARD_FIELDS.UUID]: cardId });
  ...
  async createChildSession(parentSession, parentInfo) {
    try {
      ...
      const childSession = new Play({
        ...
        status: 'active',
        state: 'swiping',
        parentSessionId: parentSession[CARD_FIELDS.UUID],
      ...
      console.log(`✅ Child session created: ${childSession[CARD_FIELDS.UUID]} for ${parentInfo.parentName}`);
      
      return {
        playId: childSession[CARD_FIELDS.UUID],
  ...
  // UTIL: Pick a comparison card from current ranking (simple median)
