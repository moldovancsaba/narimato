// FUNCTIONAL: Supervises sequential swipe-only family sessions to explore branches
// STRATEGIC: Minimizes user friction by composing proven Swipe-Only units into a larger decision tree
...
const { CARD_FIELDS, SESSION_FIELDS } = require('../constants/fields');
...
class SwipeMoreEngine {
  ...
  async createSession(organizationId, deckTag) {
    try {
      ...
      return {
        playId: swipeMoreSession[CARD_FIELDS.UUID],
        mode: 'swipe-more',
        state: 'swiping',
  ...
  async startSwipeOnlyFamily(swipeMoreSession, familyTag, level, context) {
    try {
      ...
      if (currentFamily) {
        currentFamily[SESSION_FIELDS.ID] = swipeOnlySession.playId;
  ...
  async startNextFamily(swipeMoreSession) {
    try {
      console.log(`🔄 startNextFamily called for session ${swipeMoreSession[CARD_FIELDS.UUID]}`);
      ...
      for (const rankedCard of lastFamilyResults.ranking) {
        ...
        const parentCard = await Card.findOne({
          [CARD_FIELDS.UUID]: rankedCard.card.id,
  ...
  async processVote(sessionId, cardA, cardB, winner) {
    try {
      ...
      const likedCardIds = session.likedCards.map(item => item[CARD_FIELDS.ID]);
  ...
  calculateVotingRanking(session) {
    const cards = session.likedCards.map(item => item[CARD_FIELDS.ID]);
    ...
    const rankedCardIds = cards.sort((a, b) => {
      ...
      const indexA = session.likedCards.findIndex(item => item[CARD_FIELDS.ID] === a);
      const indexB = session.likedCards.findIndex(item => item[CARD_FIELDS.ID] === b);
      return indexA - indexB;
    });
    
    // Map back to card objects with stats
    return rankedCardIds.map((cardId, index) => {
      const originalItem = session.likedCards.find(item => item[CARD_FIELDS.ID] === cardId);
      return {
        ...originalItem,
        rank: index + 1,
  ...
  async getFinalRanking(sessionId) {
    try {
      const swipeMoreSession = await SwipeMorePlay.findOne({ [CARD_FIELDS.UUID]: sessionId });
  ...
  async buildSequenceLevel(organizationId, familyTag, level, context, sequence) {
    try {
      ...
      const step = {
        ...
        expectedCards: cards.map(c => ({ id: c[CARD_FIELDS.UUID], name: c.name, title: c.title })),
      ...
      for (const parentCard of cardsWithChildren) {
        ...
        const childFamilyInfo = {
          parentCard: { id: parentCard[CARD_FIELDS.UUID], name: parentCard.name, title: parentCard.title },
          familyTag: parentCard.name,
          level: level + 1,
          context: `children-of-${parentCard.name}`
        };
    ...
    try {
      ...
      for (const likedCard of session.finalRanking) {
        ...
        if (likedCard.level === session.currentPath.length + 1) {
          const cardInTree = currentLevel.find(c => c.id === likedCard[CARD_FIELDS.ID]);
