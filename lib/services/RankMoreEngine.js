...
class RankMoreEngine {
  ...
  async handleInput(playId, { action, payload }) {
    const session = await RankMorePlay.findOne({ [fieldNames.PlayUUID]: playId, status: 'active' });
    if (!session) {
  ...
  async getNext(playId) {
    const session = await RankMorePlay.findOne({ [fieldNames.PlayUUID]: playId });
  ...
  async _onRootsCompleted(session, viaVoting) {
    ...
    if (viaVoting) {
      ...
      session.rankedRoots = res[VOTE_FIELDS.PERSONAL_RANKING] || [];
    } else {
      // Root swipe ended <2 liked -> fetch RO doc
      const ro = await RankOnlyPlay.findOne({ [fieldNames.PlayUUID]: session.activeRankOnlySession });
      session.rankedRoots = ro?.likedCardIds || [];
    }

    // Prepare first-level families: only parents with children
    const parents = await Card.find({ [fieldNames.CardUUID]: { $in: session.rankedRoots }, organizationId: session.organizationId, isActive: true });
    const withChildren = parents.filter(p => p.isParent && p.hasChildren);

    // Shuffle families (random order within the level)
    const families = withChildren.map(p => ({ parentId: p[fieldNames.CardUUID], parentName: p.name, familyTag: p.name, level: 1, status: 'pending' }));
  ...
  async _onFamilyCompleted(session) {
    ...
    const rankingIds = res[VOTE_FIELDS.PERSONAL_RANKING] || [];
    ...
    if (rankingIds.length > 0) {
      const children = await Card.find({ [fieldNames.CardUUID]: { $in: rankingIds }, organizationId: session.organizationId, isActive: true });
      ...
      const newFamilies = promotable.map(c => ({ parentId: c[fieldNames.CardUUID], parentName: c.name, familyTag: c.name, level: nextLevel, status: 'pending' }));
  ...
  async _startNextFamilyOrComplete(session) {
    ...
    fam[SESSION_FIELDS.ID] = startRes.playId;
