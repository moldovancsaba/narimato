...
// Simple onboarding adapter: right-only swipe-only; results disabled
class OnboardingAdapter {
  constructor() {
    this.mode = 'onboarding';
  }
  async startSession(organizationId, deckTag) {
    const res = await SwipeOnlyEngine.startOnboardingSession(organizationId, deckTag);
    return {
      playId: res.playId,
      initial: {
        cards: res.cards,
  ...
  async handleInput(playId, { action, payload }) {
    if (action === 'next') {
      return await (new SwipeOnlyEngine()).advanceOnboarding(playId);
    }
    if (action === 'swipe') {
      const { cardId, direction } = payload || {};
      return await SwipeOnlyEngine.processSwipe(playId, cardId, direction);
    }
    const err = new Error('Unsupported action for onboarding');
    err.statusCode = 400;
    throw err;
  }
  async getNext(playId) {
  ...
  async getResults(playId) {
    return { completed: true, ranking: [], isOnboarding: true };
  }
}
...
/**
 * Run a callback with tenant-scoped models for the play's organization.
 * Keeps engine methods inside AsyncLocalStorage (required for model proxies).
 */
async function runWithPlay(playId, fn) {
  ...
  return withOrganization(organizationId, async () => {
    const ctx = await findPlayAndEngine(playId);
    return fn(ctx);
  });
}

/** @deprecated Prefer runWithPlay so engine calls stay in tenant context */
async function getPlayAndEngine(playId) {
  return runWithPlay(playId, async (ctx) => ctx);
}

async function findPlayAndEngine(playId) {
