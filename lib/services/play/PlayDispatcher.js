...
// Engine interface adapter for Onboarding (right-only swipe-only)
...
const engines = {
  vote_only: new VoteOnlyEngineAdapter(),
...
async function getPlayAndEngine(playId) {
  ...
  if (!session) {
    const err = new Error('Play session not found');
    err.statusCode = 404;
    err.appCode = 4004;
    throw err;
  }
  const mode = session.mode || 'swipe_only';
  const engine = engines[mode];
  if (!engine) {
    const err = new Error(`Unsupported play mode: ${mode}`);
    err.statusCode = 400;
    err.appCode = 1001;
    throw err;
  }
  return { session, engine, mode };
}

module.exports = { getPlayAndEngine };