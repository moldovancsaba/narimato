...
const { CARD_FIELDS, VOTE_FIELDS } = require('../constants/fields');
...
class VoteOnlyService {
  ...
  async startSession(organizationId, deckTag) {
    ...
    const play = new Play({
      ...
      state: 'vote_only',
      status: 'active',
      cardIds: shuffled.map(c => c[CARD_FIELDS.UUID]),
      // Initialize with an empty personal ranking; the first user vote seeds it (Steps 2-3)
      personalRanking: [],
      unrankedDeck: rest.map(c => c[CARD_FIELDS.UUID]),
      rankedDeck: [a[CARD_FIELDS.UUID], b[CARD_FIELDS.UUID]],
      activeChallenger: a[CARD_FIELDS.UUID],
      activeOpponent: b[CARD_FIELDS.UUID],
      voteHistory: []
    });
  ...
  async submitVote(playId, winner, loser) {
    const play = await Play.findOne({ [CARD_FIELDS.UUID]: playId, status: 'active', mode: 'vote_only' });
    ...
    if (play[VOTE_FIELDS.PERSONAL_RANKING].length === 0) {
      // Step 2-3: rank the first pair into RANKED LIST
      play[VOTE_FIELDS.PERSONAL_RANKING] = [winner, loser];
  ...
  async getNextComparison(playId) {
    const play = await Play.findOne({ [CARD_FIELDS.UUID]: playId, mode: 'vote_only' });
    ...
    if (play.unrankedDeck && play.unrankedDeck.length > 0) {
      ...
      if (!play[VOTE_FIELDS.PERSONAL_RANKING].includes(challenger)) {
        play[VOTE_FIELDS.PERSONAL_RANKING].push(challenger);
      }
      // Reset rankedDeck to full candidate set (personal ranking) for this challenger
      play.rankedDeck = [...play[VOTE_FIELDS.PERSONAL_RANKING]];
    ...
    return { completed: true, finalRanking: play[VOTE_FIELDS.PERSONAL_RANKING] };
  }
  ...
  async getResults(playId) {
    const play = await Play.findOne({ [CARD_FIELDS.UUID]: playId });
    if (!play) throw new Error('Play session not found');

    const cards = await Card.find({ [CARD_FIELDS.UUID]: { $in: play.cardIds } });
const map = new Map(cards.map(c => [c[CARD_FIELDS.UUID], this._toCardPayload(c)]));
    ...
    return {
      ...
      ranking: play[VOTE_FIELDS.PERSONAL_RANKING].map((id, idx) => ({
        rank: idx + 1,
        cardId: id,
        card: map.get(id) || { id }
      })),
      // Provide raw IDs as a conservative fallback for legacy UI code paths
      personalRanking: play[VOTE_FIELDS.PERSONAL_RANKING],
      statistics: {
  ...
  _detectChallenger(play, winner, loser) {
    ...
    const lastPersonal = play[VOTE_FIELDS.PERSONAL_RANKING][play[VOTE_FIELDS.PERSONAL_RANKING].length - 1];
  ...
  _challengerWins(play, challenger, opponent) {
    ...
    const oppPosPR = play[VOTE_FIELDS.PERSONAL_RANKING].indexOf(opponent);
Of(opponent);
    if (oppPosPR >= 0) {
      const removeSet = new Set(play[VOTE_FIELDS.PERSONAL_RANKING].filter((id, idx) => idx >= oppPosPR));
    ...
    const oppPos = play[VOTE_FIELDS.PERSONAL_RANKING].indexOf(opponent);
    if (oppPos >= 0) {
      const chalPos = play[VOTE_FIELDS.PERSONAL_RANKING].indexOf(challenger);
      if (chalPos === -1) play[VOTE_FIELDS.PERSONAL_RANKING].push(challenger);
      const curPos = play[VOTE_FIELDS.PERSONAL_RANKING].indexOf(challenger);
      play[VOTE_FIELDS.PERSONAL_RANKING].splice(curPos, 1);
      play[VOTE_FIELDS.PERSONAL_RANKING].splice(oppPos, 0, challenger);
    }
  }
