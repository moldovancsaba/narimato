...
class VoteMoreEngine {
  ...
  async createSession(organizationId, deckTag) {
    ...
    if (currentFamily) {
      currentFamily[SESSION_FIELDS.ID] = startRes.playId;
  ...
  async processVote(sessionId, winner, loser) {
    const session = await VoteMorePlay.findOne({ [CARD_FIELDS.UUID]: sessionId, status: 'active' });
  ...
  async getNext(sessionId) {
    ...
    if (!session.activeVoteOnlySession) {
      ...
      if (session.currentFamilyIndex < session.familiesToProcess.length) {
        ...
        session.activeVoteOnlySession = startRes.playId;
        nextFamily[SESSION_FIELDS.ID] = startRes.playId;
  ...
  async enqueueChildrenFamilies(session, familyResults) {
    ...
    for (const item of (familyResults.ranking || [])) {
      const cardId = item.card?.id || item[CARD_FIELDS.ID];
      if (!cardId) continue;
      const parentCard = await Card.findOne({ [CARD_FIELDS.UUID]: cardId, organizationId: session.organizationId, isActive: true });
  ...
  async getFinalRanking(sessionId) {
    const session = await VoteMorePlay.findOne({ [CARD_FIELDS.UUID]: sessionId });
