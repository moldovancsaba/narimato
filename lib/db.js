...
function getMasterPipelineJobModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerPipelineJobModel } = require('./models/PipelineJob');
  return registerPipelineJobModel(masterConnection);
}

function getMasterTopicSpecModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerTopicSpecModel } = require('./models/TopicSpec');
  return registerTopicSpecModel(masterConnection);
}

function getMasterGlobalSettingModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerGlobalSettingModel } = require('./models/GlobalSetting');
  return registerGlobalSettingModel(masterConnection);
}

function getMasterDeckIntelligenceConfigModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerDeckIntelligenceConfigModel } = require('./models/DeckIntelligenceConfig');
  return registerDeckIntelligenceConfigModel(masterConnection);
}

function getMasterSourceModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerSourceModel } = require('./models/Source');
  return registerSourceModel(masterConnection);
}

function getMasterIntelligenceMemoryModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerIntelligenceMemoryModel } = require('./models/IntelligenceMemory');
  return registerIntelligenceMemoryModel(masterConnection);
}

function getMasterIntelligenceRuleModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerIntelligenceRuleModel } = require('./models/IntelligenceRule');
  return registerIntelligenceRuleModel(masterConnection);
}

function getMasterIntelligencePersonaModel() {
  if (!masterConnection) {
    throw new Error('Master connection not initialized. Call connectMaster() first.');
  }
  const { registerIntelligencePersonaModel } = require('./models/IntelligencePersona');
  return registerIntelligencePersonaModel(masterConnection);
}
