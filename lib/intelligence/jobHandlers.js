...
function buildGenerationPrompt(topicSpec) {
  const c = topicSpec.planningConstraints || {};
  return `You are a Narimato deck architect. Generate a JSON object with key "cards" (array).
  ...
  switch (type) {
    ...
    case JOB_TYPES.REGENERATE_TAG: {
      ...
      if (process.env.OLLAMA_SKIP === '1' || payload.useFixture) {
        ...
        const raw = await queueAiInference(() =>
          ollamaGenerate(buildGenerationPrompt(topicSpec))
        );
    ...
    case JOB_TYPES.RECONCILE_PLAY_FEEDBACK: {
      ...
      return {
        ...
        enabled: true,
        playId: payload.playId,
        deckRootTag: payload.deckRootTag,
        mode: payload.mode,
        message: 'v2 play→corpus reconciliation not implemented; job logged for future ADR',
      };
    }
