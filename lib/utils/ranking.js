// FUNCTIONAL: Ranking utilities (binary search with accumulated bounds + global ELO updates)
// STRATEGIC: Provides deterministic personal ranking and system-wide ELO with minimal coupling
...
const { CARD_FIELDS } = require('../constants/fields');
...
async function updateGlobalRankings(winnerId, loserId) {
  ...
  const [winner, loser] = await Promise.all([
    Card.findOne({ [fieldNames.CardUUID]: winnerId }),
    Card.findOne({ [fieldNames.CardUUID]: loserId })
  ]);
...
async function getFamilyInfo(cardId) {
  const Card = require('../models/Card');
  const card = await Card.findOne({ [fieldNames.CardUUID]: cardId });
  if (!card) return null;
...
async function getFamilyAwareNextComparison(personalRanking, newCardId, allVotes = []) {
  ...
  const rankingFamilies = await Promise.all(
    personalRanking.map(async cardId => {
      ...
      return { [CARD_FIELDS.ID]: cardId, family: family?.family, isParent: family?.isParent };
    })
  );
  ...
  const familyRanking = sameFamilyCards.map(card => card[CARD_FIELDS.ID]);
  ...
  if (sameFamilyCards.length === 0) {
    ...
    if (newCardFamily.isParent) {
      ...
      const parentIndex = ranking.findIndex(cardId => {
        const card = rankingFamilies.find(f => f[CARD_FIELDS.ID] === cardId);
